...
var (
	...
	slimResTag            = dependencyTag{name: "org.slim.framework-res"}
...
func (j *Module) deps(ctx android.BottomUpMutatorContext) {
	if ctx.Device() {
		...
		if (ctx.ModuleName() == "framework") || (ctx.ModuleName() == "framework-annotation-proc") {
			...
			ctx.AddDependency(ctx.Module(), slimResTag, "org.slim.framework-res")
		}
		...
		if ctx.ModuleName() == "org.slim.framework-res" {
			ctx.AddDependency(ctx.Module(), frameworkResTag, "framework-res")
		}
		if ctx.ModuleName() == "org.slim.framework" {
			ctx.AddDependency(ctx.Module(), slimResTag, "org.slim.framework-res")
		}
	}
...
func (j *Module) collectDeps(ctx android.ModuleContext) deps {
	...
	ctx.VisitDirectDeps(func(module android.Module) {
		...
		case Dependency:
			...
			case slimResTag:
				if ctx.ModuleName() == "org.slim.framework" {
					// org.slim.framework.jar has a one-off dependency on the R.java and Manifest.java files
					// generated by org.slim.framework-res.apk
					deps.srcJars = append(deps.srcJars, dep.(*AndroidApp).aaptSrcJar)
				}
